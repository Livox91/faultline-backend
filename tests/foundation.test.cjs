...
test('shared packages resolve at runtime and telemetry validates envelopes', () => {
  ...
  const envelope = { id: 'event-1', clusterId: 'cluster-1', kind: 'example',
    observedAt: '2026-09-08T00:00:00Z', payload: {} };
  assert.equal(telemetryEnvelopeSchema.safeParse(envelope).success, true);
  assert.equal(telemetryEnvelopeSchema.safeParse({ ...envelope, observedAt: 'bad' }).success, false);
  const { payload, ...missingPayload } = envelope;
  assert.equal(telemetryEnvelopeSchema.safeParse(missingPayload).success, false);
...
for (const [index, app] of ['api', 'ingestion', 'processor'].entries()) {
  test(`${app}: validates required settings and app-specific defaults`, () => {
    ...
    assert.throws(() => validate({ ...validEnvironment, APP_VERSION: '  ' }), /APP_VERSION/);
    assert.throws(() => validate({ ...validEnvironment, HOST: '' }), /HOST/);
    assert.throws(() => validate({ ...validEnvironment, LOG_LEVEL: 'secret-value' }),
      (error) => error.message.includes('LOG_LEVEL') && !error.message.includes('secret-value'));
    assert.throws(() => validate({ ...validEnvironment, NODE_ENV: 'invalid' }), /NODE_ENV/);
  });

  test(`${app}: resolves all shared packages from its workspace`, () => {
    const requireFromApp = createRequire(resolve(root, `apps/${app}/package.json`));
    for (const name of ['platform', 'telemetry', 'kubernetes', 'incidents', 'database', 'queue']) {
      assert.doesNotThrow(() => requireFromApp(`@faultline/${name}`));
    }
  });

  test(`${app}: invalid configuration exits with a structured error`, { timeout: 35000 }, () => {
    const result = spawnSync(process.execPath, [resolve(root, `apps/${app}/dist/main.js`)], {
      cwd: root,
    ...
    const logs = (result.stdout + result.stderr).trim().split(/\r?\n/).map(JSON.parse);
    assert.ok(logs.some((entry) => entry.application === app &&
      entry.level === 'error' && entry.message.includes('APP_VERSION')));
  });

  test(`${app}: independently boots and serves health with JSON logs`, { timeout: 45000 }, async () => {
    const port = await freePort();
    const child = spawn(process.execPath, [resolve(root, `apps/${app}/dist/main.js`)], {
      cwd: root,
      env: { ...process.env, ...validEnvironment, HOST: '127.0.0.1', PORT: String(port), LOG_LEVEL: 'log' },
    ...
    try {
      ...
      if (app === 'api') {
        ...
        assert.deepEqual(await info.json(), {
          application: 'api', environment: 'test', version: validEnvironment.APP_VERSION,
      ...
      assert.ok(logs.some((entry) => entry.message?.event === 'application_started'));
      assert.ok(logs.every((entry) => entry.application === app &&
        Number.isFinite(entry.timestamp) && typeof entry.level === 'string' &&
...
test('health service reports application and process uptime', (context) => {
  ...
  assert.deepEqual(health.getStatus(), { application: 'processor', status: 'ok', uptime: 42.5 });
});
...
test('logger retains application across contexts and filters levels', () => {
  ...
  assert.ok(logs.every((entry) => entry.application === 'ingestion' &&
    entry.context === 'SharedLibrary' && Number.isFinite(entry.timestamp)));
...
const metadata = {
  ...
  node: 'worker-1', service: 'api', workload: 'api',
  attributes: { source: 'test', nested: { count: 2 }, tags: ['a'] },
  raw: { original: true },
};
const events = [
  { ...metadata, kind: 'log', level: 'info', message: 'Started' },
  { ...metadata, kind: 'metric', name: 'cpu_usage', value: 0.2, unit: 'cores', metricType: 'gauge' },
  { ...metadata, kind: 'kubernetes', type: 'Warning', reason: 'BackOff', message: 'Restarting',
    involvedObject: { clusterId: 'cluster-1', apiVersion: 'v1', kind: 'Pod', name: 'api-1', namespace: 'default' },
    count: 1 },
];
...
test('telemetry validates all event variants and preserves common metadata', () => {
  ...
  const { namespace, pod, container, node, service, workload, attributes, ...clusterEvent } = events[0];
  assert.deepEqual(telemetryEventSchema.parse({ ...clusterEvent, raw: null }).attributes, {});
});

test('telemetry rejects malformed events at the shared boundary', () => {
  for (const key of ['id', 'timestamp', 'clusterId', 'raw', 'kind', 'level', 'message']) {
    const missing = { ...events[0] };
    delete missing[key];
    assert.equal(telemetryEventSchema.safeParse(missing).success, false, key);
  }
  for (const patch of [
    { timestamp: 'not-a-date' }, { timestamp: '2026-09-08T10:00:00' },
    { id: '  ' }, { clusterId: '' }, { namespace: '' },
    { level: 'invalid' }, { kind: 'unknown' }, { raw: undefined },
    { attributes: { callback: () => {} } },
  ]) {
    assert.equal(telemetryEventSchema.safeParse({ ...events[0], ...patch }).success, false);
  }
  for (const value of [NaN, Infinity, '1']) {
    assert.equal(telemetryEventSchema.safeParse({ ...events[1], value }).success, false);
  }
  assert.equal(telemetryEventSchema.safeParse({ ...events[2], count: 0 }).success, false);
  assert.equal(telemetryEventSchema.safeParse({
    ...events[2], involvedObject: { ...events[2].involvedObject, clusterId: 'another-cluster' },
  }).success, false);
});
